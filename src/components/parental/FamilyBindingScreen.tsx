...
export function FamilyBindingScreen({ session, onBound }: Props) {
  ...
  const createFamily = async () => {
    ...
    //  2) 4 nhãn đặc biệt "Ưu tiên"/"Ẩn"/"Mobile"/"TV" (is_builtin) — không có thì Trang chủ
    //     mất các tính năng ưu tiên/ẩn/giới hạn theo thiết bị, và khu Bố mẹ > Quản lý nhãn
    //     cũng thiếu các nhãn mặc định mọi gia đình khác đều có (xem
    //     supabase/010_content_labels.sql + 019_device_visibility_labels.sql, seed gốc).
    await supabase.from('content_labels').insert([
      ...
      { name: 'Mobile', is_priority: false, is_hidden: false, is_phone_only: true, is_desktop_only: false, is_builtin: true },
      { name: 'TV', is_priority: false, is_hidden: false, is_phone_only: false, is_desktop_only: true, is_builtin: true },
